...
window.shower = window.shower || (function(window, document, undefined) {
	...
	Slide.prototype = {
		 ...
		 */
		isFinished : function() {
			return (this.innerComplete >= this.innerLength) && (this.innerComplete >= this.innerLength2);
		},
		...
		prev : function(shower) {
			...
			if ( (! slide.hasInnerNavigation && ! slide.hasInnerNavigation2) || slide.isFinished() || slide.innerComplete === 0) {
				shower.prev();
				return false;
			}
		...
		next : function(shower) {
			...
			if ( (! slide.hasInnerNavigation && ! slide.hasInnerNavigation2) || slide.isFinished()) {
				shower.next();
				return false;
			}
			...
			if ( ! slide.isFinished()) {
				...
				nextSteps2 = document.getElementById(slide.id).querySelectorAll('.highlight');
				if (nextSteps2.length) {
					if (slide.innerComplete > 0) {
						nextSteps2[slide.innerComplete - 1].classList.remove('active');
					}
					nextSteps2[slide.innerComplete].classList.add('active');
				}
	...
	shower.init = function(slideSelector, progressSelector) {
		...
		for (var i = 0; i < slides.length; i++) {
			...
			shower.slideList.push(new Slide({
				...
				hasInnerNavigation2 : null !== slides[i].querySelector('.highlight'),
				timing : timing,
				innerLength : slides[i].querySelectorAll('.next').length,
				innerLength2 : slides[i].querySelectorAll('.highlight').length,
				innerComplete : 0
			}));
		}
