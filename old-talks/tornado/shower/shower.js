	...
	shower.increaseInnerNavigation = function(slideNumber) {
		...
		if (slideList[slideNumber].hasInnerNavigation) {
			...
			if (0 !== nextNodes.length) {
				...
				if (slideList[slideNumber].id=='new-schema') {
					if (node.className.indexOf('last') != -1) {
						var firstNode = document.getElementById(slideList[slideNumber].id).querySelector('.first');
						firstNode.setAttribute('style', 'display:none');
					} else if (node.className.indexOf('remove') != -1) {
						var secondNode = document.getElementById(slideList[slideNumber].id).querySelector('.last');
						secondNode.setAttribute('style', 'display:none');
					}
				}
				return true;
			}
